        ...
        for season in self.load_factors.columns:
            ...
            for time_step in range(self.load_factors.shape[0]):
                ...
                try:
                    ...
                    slack_bus_load = pd.DataFrame(
                        [[0, 0, 0]], columns=["bus", "p_mw", "q_mvar"]
                    )
                    load_data = pd.concat(
                        [slack_bus_load, load_data], ignore_index=True
                    )
                    lfa_results = {
                        "res_bus": deepcopy(self.net.res_bus.values),
                        "load": deepcopy(load_data.values.astype(float)),
                        "res_line": deepcopy(
                            self.net.res_line[self.net.line["in_service"]].values
                        ),
                    }
        ...
        with h5py.File("raw_data/33_bus_results.h5", "w") as f:
            for net_id, net_data in self.all_results.items():
                ...
                static_group.create_dataset(
                    "line", data=net_data["network_config"]["line"]
                )
                static_group.create_dataset(
                    "bus", data=net_data["network_config"]["bus"]
                )
                for season, time_step_data in net_data.items():
                    ...
                    for time_step, results in time_step_data.items():
                        time_step_group = season_group.create_group(
                            f"time_step_{time_step}"
                        )
                        time_step_group.create_dataset(
                            "res_bus", data=results["res_bus"]
                        )
                        time_step_group.create_dataset(
                            "res_line", data=results["res_line"]
                        )
